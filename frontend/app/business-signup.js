...
export default function BusinessSignupScreen() {
  ...
  const handleSignup = async () => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/api/auth/business/signup', { name: form.name.trim(), email: form.email.trim(), password: form.password });
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.scrollContent}>
      ...
      <View style={styles.form}>
        ...
        <Text style={styles.subtitle}>Register your brand on PLXYGROUND</Text>

        <View style={styles.brandBadge}><Text style={styles.brandBadgeText}>🏢 Business Account</Text></View>

        <Text style={styles.label}>Organization Name</Text>
        <TextInput style={styles.input} value={form.name} onChangeText={v => setForm(p => ({...p, name: v}))} placeholder="Your company name" placeholderTextColor="#9ca3af" accessibilityLabel="Organization name" />

        <Text style={styles.label}>Business Email</Text>
        <TextInput style={styles.input} value={form.email} onChangeText={v => setForm(p => ({...p, email: v}))} placeholder="contact@company.com" placeholderTextColor="#9ca3af" keyboardType="email-address" autoCapitalize="none" accessibilityLabel="Business email" />
        ...
        <TouchableOpacity style={styles.checkboxRow} onPress={() => setForm(p => ({...p, agreeTerms: !p.agreeTerms}))} accessibilityRole="checkbox" accessibilityState={{ checked: form.agreeTerms }}>
          ...
          <TouchableOpacity onPress={() => router.push('/terms')} accessibilityRole="link"><Text style={styles.link}>Terms</Text></TouchableOpacity>
        ...
        <TouchableOpacity style={[styles.submitBtn, loading && styles.submitBtnDisabled]} onPress={handleSignup} disabled={loading} accessibilityRole="button">
          {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.submitBtnText}>Create Business Account</Text>}
        </TouchableOpacity>

        <View style={styles.altRow}>
          <Text style={styles.altText}>Already registered? </Text>
        ...
        <View style={styles.altRow}>
          <Text style={styles.altText}>Are you a creator? </Text>
