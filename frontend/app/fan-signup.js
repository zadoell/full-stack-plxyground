...
export default function FanSignupScreen() {
  ...
  const handleSignup = async () => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/api/auth/fan/signup', { name: form.name.trim(), email: form.email.trim(), password: form.password });
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.scrollContent}>
      ...
      <View style={styles.form}>
        <Text style={styles.badge}>🏟️ Fan Account</Text>
        <Text style={styles.title}>Join as a Fan</Text>
        <Text style={styles.subtitle}>Follow your favorite creators and discover great sports content</Text>
        ...
        <TouchableOpacity style={[styles.submitBtn, loading && styles.submitBtnDisabled]} onPress={handleSignup} disabled={loading} accessibilityRole="button" accessibilityLabel="Create fan account">
...
const styles = StyleSheet.create({
  ...
  backBtnText: { color: '#00E676', fontSize: 14, fontWeight: '600' },
  form: { maxWidth: 440, width: '100%', alignSelf: 'center' },
  badge: { fontSize: 14, color: '#00E676', fontWeight: '600', marginBottom: 10, backgroundColor: 'rgba(0,230,118,0.08)', alignSelf: 'flex-start', paddingHorizontal: 14, paddingVertical: 5, borderRadius: 6, overflow: 'hidden', borderWidth: 1, borderColor: 'rgba(0,230,118,0.25)' },
  ...
  input: { borderWidth: 1.5, borderColor: 'rgba(0,230,118,0.2)', borderRadius: 10, paddingHorizontal: 16, paddingVertical: 14, fontSize: 15, color: '#F0F0FA', backgroundColor: 'rgba(0,230,118,0.04)' },
  ...
  submitBtn: { backgroundColor: '#00E676', paddingVertical: 16, borderRadius: 10, alignItems: 'center', marginTop: 28, shadowColor: '#00E676', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.12, shadowRadius: 12 },
