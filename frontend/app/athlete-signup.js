...
export default function AthleteSignupScreen() {
  ...
  const [form, setForm] = useState({ name: '', email: '', password: '', sport: '', agreeTerms: false });
  ...
  const handleSignup = async () => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/api/auth/athlete/signup', { name: form.name.trim(), email: form.email.trim(), password: form.password });
      await login(res.token, res.user);
      showToast('Welcome to PLXYGROUND!', 'success');
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.scrollContent}>
      ...
      <View style={styles.form}>
        <Text style={styles.badge}>🏆 Athlete Account</Text>
        <Text style={styles.title}>Join as an Athlete</Text>
        <Text style={styles.subtitle}>Showcase your training, compete, and connect with brands and fans</Text>

        <Text style={styles.label}>Full Name</Text>
        <TextInput style={styles.input} value={form.name} onChangeText={v => setForm(p => ({...p, name: v}))} placeholder="Your name" placeholderTextColor="#9ca3af" accessibilityLabel="Full name" />
        ...
        <Text style={styles.label}>Sport / Discipline (optional)</Text>
        <TextInput style={styles.input} value={form.sport} onChangeText={v => setForm(p => ({...p, sport: v}))} placeholder="e.g. Basketball, Track & Field" placeholderTextColor="#9ca3af" accessibilityLabel="Sport or discipline" />
        ...
        <TouchableOpacity style={[styles.submitBtn, loading && styles.submitBtnDisabled]} onPress={handleSignup} disabled={loading} accessibilityRole="button" accessibilityLabel="Create athlete account">
          {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.submitBtnText}>Join PLXYGROUND</Text>}
        </TouchableOpacity>
        ...
        <View style={styles.altRow}>
          <Text style={styles.altText}>Want to create content? </Text>
          <TouchableOpacity onPress={() => router.push('/signup')} accessibilityRole="link"><Text style={styles.link}>Creator Signup</Text></TouchableOpacity>
        </View>
      </View>
    </ScrollView>
  );
...
const styles = StyleSheet.create({
  ...
  backBtnText: { color: '#FFD100', fontSize: 14, fontWeight: '600' },
  form: { maxWidth: 440, width: '100%', alignSelf: 'center' },
  badge: { fontSize: 14, color: '#FFD100', fontWeight: '600', marginBottom: 10, backgroundColor: 'rgba(255,209,0,0.08)', alignSelf: 'flex-start', paddingHorizontal: 14, paddingVertical: 5, borderRadius: 6, overflow: 'hidden', borderWidth: 1, borderColor: 'rgba(255,209,0,0.25)' },
  ...
  input: { borderWidth: 1.5, borderColor: 'rgba(255,209,0,0.2)', borderRadius: 10, paddingHorizontal: 16, paddingVertical: 14, fontSize: 15, color: '#F0F0FA', backgroundColor: 'rgba(255,209,0,0.04)' },
  checkboxRow: { flexDirection: 'row', alignItems: 'center', marginTop: 22, flexWrap: 'wrap' },
  checkbox: { width: 24, height: 24, borderWidth: 2, borderColor: 'rgba(255,255,255,0.15)', borderRadius: 6, marginRight: 10, justifyContent: 'center', alignItems: 'center' },
  ...
  submitBtn: { backgroundColor: '#FFD100', paddingVertical: 16, borderRadius: 10, alignItems: 'center', marginTop: 28, shadowColor: '#FFD100', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.12, shadowRadius: 12 },
